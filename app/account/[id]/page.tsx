...
export default function AccountPage() {
  ...
  // FIX 4: single animated value state, updated once after data loads
  ...
  // FIX 4: slowed down animation — runs requestAnimationFrame instead of setInterval
  // and only triggers once, not three overlapping timers at 50fps each
  const animateValue = (value: number, setter: (v: number) => void) => {
    if (value === 0) return;
    const duration = 800;
    const start = performance.now();
    const tick = (now: number) => {
      const progress = Math.min((now - start) / duration, 1);
      setter(Math.floor(progress * value));
  ...
  useEffect(() => {
    ...
    const loadPageData = async () => {
      ...
      try {
        ...
        // FIX 1: ALL queries in a single Promise.all — no second fetch outside this block
        const [
          { data: profileData },
          { data: productData },
          { count: followers },
          ...
          { data: ratingsData },
          { data: payoutData },
          // FIX 1: merged the orphaned ordersData fetch into this batch
          { data: ordersData },
        ] = await Promise.all([
          ...
          supabase.from("orders")
            .select("*, products(title, image_url, price)")
            .eq("buyer_id", id)
            .in("status", ["paid", "payment_held", "completed", "delivered"]),
          ...
          // FIX 1: was fetched separately after the try/catch — now part of the batch
          supabase.from("orders").select("*").eq("seller_id", id),
        ]);
        ...
        // FIX 1: process ordersData here instead of in a separate useEffect/fetch
        if (ordersData) {
          ...
          // FIX 4: animate after all state is set, using rAF instead of setInterval
            ...
            >
              ...
              <div className="space-y-5">
                <div className="border-b border-[#2B0A0F]/12 focus-within:border-[#2B0A0F]/40 transition-colors">
                  ...
                  <input type="text" value={bankUpi} onChange={(e) => setBankUpi(e.target.value)} placeholder="yourname@upi"
                ...
                <div className="border-b border-[#2B0A0F]/12 focus-within:border-[#2B0A0F]/40 transition-colors">
                  ...
                  <input type="text" value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder="As on bank account"
                    className="w-full bg-transparent pb-3 outline-none text-base placeholder:opacity-20" />
                </div>
                <div className="border-b border-[#2B0A0F]/12 focus-within:border-[#2B0A0F]/40 transition-colors">
                  <label className="text-[8px] uppercase tracking-[0.25em] opacity-40 block mb-2">Account Number</label>
                  <input type="text" value={bankAccount} onChange={(e) => setBankAccount(e.target.value)} placeholder="Bank account number"
                    className="w-full bg-transparent pb-3 outline-none text-base placeholder:opacity-20" />
                </div>
                <div className="border-b border-[#2B0A0F]/12 focus-within:border-[#2B0A0F]/40 transition-colors">
                  <label className="text-[8px] uppercase tracking-[0.25em] opacity-40 block mb-2">IFSC Code</label>
                  <input type="text" value={bankIfsc} onChange={(e) => setBankIfsc(e.target.value.toUpperCase())} placeholder="e.g. HDFC0001234"
                    className="w-full bg-transparent pb-3 outline-none text-base placeholder:opacity-20" />
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-10 md:gap-16 mb-8 sm:mb-12">
          ...
          <div className="relative group flex-shrink-0">
            <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-36 md:h-36 rounded-full bg-[#2B0A0F] flex items-center justify-center text-[#F6F3EF] text-3xl sm:text-4xl font-light overflow-hidden border-4 border-white shadow-md">
              {user.avatar_url ? (
                // FIX 5: added sizes prop so Next.js serves a properly sized avatar image
                <Image
                  src={user.avatar_url}
                  alt={displayName}
                  fill
              ...
              <div className="flex items-center gap-2 justify-center sm:justify-start flex-wrap sm:ml-2">
                {isOwner ? (
                  <>
                    ...
                    <Link href="/settings">
                      <button className="px-3 py-2 rounded-full border border-[#2B0A0F]/20 text-[10px] uppercase tracking-[0.18em] hover:bg-[#2B0A0F] hover:text-[#F6F3EF] transition-all" title="Settings">
                        ⚙
                      </button>
                    </Link>
                    <button
