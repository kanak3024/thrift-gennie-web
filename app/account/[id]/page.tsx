...
function AccountSkeleton() {
  return (
    <main className="min-h-screen bg-[#F6F3EF] pt-32 pb-20 px-6">
      <div className="max-w-4xl mx-auto animate-pulse">
        <div className="flex gap-10 mb-14">
          <div className="w-36 h-36 rounded-full bg-[#EAE3DB] flex-shrink-0" />
          <div className="flex-1 space-y-4 pt-4">
            <div className="h-7 w-48 bg-[#EAE3DB] rounded-full" />
            <div className="h-3 w-64 bg-[#EAE3DB] rounded-full" />
            <div className="h-3 w-40 bg-[#EAE3DB] rounded-full" />
          </div>
        </div>
...
function Toast({ message, type }: { message: string; type: "success" | "error" }) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full text-[10px] uppercase tracking-[0.2em] shadow-lg ${
        type === "success"
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-[#F6F3EF] rounded-2xl p-8 w-[340px] shadow-2xl"
      >
...
export default function AccountPage() {
  ...
  const handleMarkUnavailable = async (productId: string) => {
    ...
    if (!error) {
      setProducts(prev => prev.map(p => p.id === productId ? { ...p, status: "unavailable" } : p));
      setOpenMenuId(null);
    }
  };
  ...
  const handleMarkAvailable = async (productId: string) => {
    ...
    if (!error) {
      setProducts(prev => prev.map(p => p.id === productId ? { ...p, status: "available" } : p));
      setOpenMenuId(null);
    }
  };
  ...
  const handleDelete = async (productId: string) => {
    ...
    if (!error) {
      setProducts(prev => prev.filter(p => p.id !== productId));
      showToast("Piece removed from archive");
    } else {
  ...
  if (!user) return (
    <main className="min-h-screen bg-[#F6F3EF] flex flex-col items-center justify-center gap-6 px-6">
      <p className="text-3xl opacity-20" style={{ fontFamily: "var(--font-playfair)" }}>
        Curator not found.
      </p>
      ...
      <div className="pt-32 pb-0 px-6 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-10 md:gap-16 mb-12">
          ...
          <div className="relative group flex-shrink-0">
            <div className="w-28 h-28 md:w-36 md:h-36 rounded-full bg-[#2B0A0F] flex items-center justify-center text-[#F6F3EF] text-4xl font-light overflow-hidden border-4 border-white shadow-md">
              {user.avatar_url ? (
          ...
          <div className="flex-1 text-center md:text-left w-full">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 mb-5">
              {isEditing ? (
                <input
                  className="bg-transparent border-b border-[#2B0A0F]/20 outline-none pb-1 w-full max-w-sm"
                ...
                <h1
                  ...
                  style={{ fontFamily: "var(--font-playfair)", fontSize: "clamp(1.8rem,3vw,2.5rem)" }}
                >
                  {displayName}
                </h1>
              )}
                ...
                {isOwner ? (
                  <>
                    <button
                      ...
                      className={`px-5 py-2 rounded-full text-[10px] uppercase tracking-[0.18em] transition-all ${
                        isEditing
                    ...
                    {isEditing && (
                      <button
                        ...
                        className="px-5 py-2 rounded-full border border-[#2B0A0F]/15 text-[10px] uppercase tracking-[0.18em] opacity-50 hover:opacity-100 transition-opacity"
                      >
                        Cancel
                      </button>
                    )}
                    <Link href="/orders">
                      <button className="px-5 py-2 rounded-full border border-[#2B0A0F]/20 text-[10px] uppercase tracking-[0.18em] hover:bg-[#2B0A0F] hover:text-[#F6F3EF] transition-all">
                        Orders
                      </button>
                    </Link>
                  </>
                  ...
                  currentUser && (
                    <motion.button
                      ...
                      className={`px-6 py-2.5 rounded-full text-[10px] uppercase tracking-[0.18em] transition-all disabled:opacity-40 ${
                        isFollowing
                          ? "border border-[#2B0A0F]/20 hover:bg-[#A1123F]/08 hover:border-[#A1123F]/30 hover:text-[#A1123F]"
                          : "bg-[#2B0A0F] text-[#F6F3EF] hover:opacity-80"
                      }`}
                    >
            ...
            <div className="flex justify-center md:justify-start gap-8 py-4 border-y border-[#2B0A0F]/08 mb-5">
              {[
                ...
                <div key={stat.label} className="text-center md:text-left">
                  ...
                  <div className="text-[9px] uppercase tracking-[0.2em] opacity-40 mt-0.5">{stat.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="max-w-sm mx-auto md:mx-0 space-y-2">
              {isEditing ? (
                <>
                  <input
                    className="w-full text-[10px] uppercase tracking-[0.25em] text-[#B48A5A] bg-transparent border-b border-[#2B0A0F]/10 outline-none pb-1"
                    value={editRole}
        ...
        {isOwner && (
          <div className="mb-6 p-6 bg-white rounded-2xl border border-[#2B0A0F]/06">
            <div className="flex justify-between items-center">
              ...
              >
                {showBankForm ? "Cancel" : user.bank_account_number ? "Edit" : "Add Details"}
              </button>
            </div>
        ...
        {/* ══════════════════════════════
            TABS
        ══════════════════════════════ */}
        <div className="flex gap-1 border-b border-[#2B0A0F]/08">
          {([
                          ...
                          <AnimatePresence>
                            {openMenuId === product.id && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, scale: 1, y: 0 }}
                                exit={{ opacity: 0, scale: 0.95 }}
                                className="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-[#2B0A0F]/08 shadow-xl overflow-hidden text-[10px] uppercase tracking-[0.15em]"
                              >
                                {product.status !== "sold" && (
            ...
            <motion.div key="sold" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              {soldItems.length === 0 ? (
                ...
                <div className="space-y-2">
                  {soldItems.map((order) => (
                    <div key={order.id} className="flex items-center gap-5 p-4 bg-white rounded-2xl border border-[#2B0A0F]/06 hover:border-[#2B0A0F]/15 transition-colors">
                      <div className="relative w-14 h-16 flex-shrink-0 overflow-hidden rounded-lg bg-[#EAE3DB]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[9px] uppercase tracking-[0.2em] opacity-35 mt-1">
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <span className="text-[8px] uppercase tracking-[0.2em] text-[#6B7E60] bg-[#6B7E60]/10 px-2.5 py-1 rounded-full mt-1 inline-block">
                          Sold
                        </span>
                      </div>
                    </div>
                ...
                <div className="space-y-2">
                  {purchasedItems.map((order) => (
                    <Link key={order.id} href={`/product/${order.product_id}`}>
                      <div className="flex items-center gap-5 p-4 bg-white rounded-2xl border border-[#2B0A0F]/06 hover:border-[#2B0A0F]/15 transition-all group">
                        <div className="relative w-14 h-16 flex-shrink-0 overflow-hidden rounded-lg bg-[#EAE3DB]">
                          <Image src={order.products?.image_url || "/final.png"} alt={order.products?.title || "Product"} fill className="object-cover" />
                        </div>
                        <div className="flex-1 min-w-0">
                        ...
                        <div className="text-right flex-shrink-0 flex items-center gap-3">
                          <div>
                            ...
                            <span className="text-[8px] uppercase tracking-[0.2em] text-[#B48A5A] bg-[#B48A5A]/10 px-2.5 py-1 rounded-full mt-1 inline-block">
                              Purchased
                            </span>
                          </div>
                          <span className="opacity-0 group-hover:opacity-40 transition-opacity text-sm">→</span>
                        </div>
                      </div>
                    </Link>
                  ))}
