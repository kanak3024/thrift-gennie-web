...
export default function SellerSheet({ sellerId, onClose }: Props) {
  ...
  // fetch seller profile when sellerId changes
  useEffect(() => {
    if (!sellerId) return;
    setLoading(true);
    setSeller(null);
    ...
    ]).then(([profileRes, listingsRes, salesRes]) => {
      if (profileRes.data) {
          ...
          >
            ...
            {loading ? (
              <div className="px-6 py-10 flex flex-col items-center gap-3">
                <div className="w-14 h-14 rounded-full bg-white/05 animate-pulse" />
                <div className="w-32 h-3 rounded-full bg-white/05 animate-pulse" />
              ...
              <div className="px-6 pt-3 pb-6">
                ...
                <div className="flex items-center gap-4 mb-5">
                  <div
                    className="w-14 h-14 rounded-full flex-shrink-0 flex items-center justify-center overflow-hidden border-2 border-white/10"
                  ...
                  >
                    {seller.avatar_url ? (
                      ...
                      <span
                        ...
                        style={{ color: "#F6F3EF", fontFamily: "var(--font-dm)" }}
                      >
                  ...
                  <div>
                    <p
                      className="text-[#F6F3EF] text-lg leading-tight"
                    ...
                    {seller.location && (
                      <p
                        className="text-[10px] text-white/30 mt-0.5 flex items-center gap-1"
                        style={{ fontFamily: "var(--font-dm)" }}
                      >
                ...
                <div className="grid grid-cols-3 gap-0 mb-5 border border-white/08 rounded-2xl overflow-hidden">
                  {[
                    ...
                    <div
                      key={stat.label}
                      className={`flex flex-col items-center py-3 ${i < 2 ? "border-r border-white/08" : ""}`}
                    >
                      <span
                        className="text-[#F6F3EF] text-lg"
                        style={{ fontFamily: "var(--font-cormorant)", fontWeight: 500 }}
                      >
                        {stat.num}
                      </span>
                      <span
                        className="text-[10px] text-white/35 uppercase tracking-[0.15em] mt-0.5"
                        style={{ fontFamily: "var(--font-dm)" }}
                      >
                        {stat.label}
                      </span>
                ...
                {seller.bio && (
                  <p
                    className="text-[13px] text-white/55 leading-relaxed mb-5"
                    style={{ fontFamily: "var(--font-dm)" }}
                  >
                    {seller.bio}
                  </p>
                ...
                <div className="flex gap-3">
                  {currentUserId && currentUserId !== seller.id ? (
                    <button
                      ...
                      className="flex-1 py-3 rounded-full text-[11px] uppercase tracking-[0.18em] font-medium transition-all"
                      style={{
                  ...
                  <Link href={`/messages?seller=${seller.id}`} className={currentUserId && currentUserId !== seller.id ? "" : "flex-1"}>
                    <button
